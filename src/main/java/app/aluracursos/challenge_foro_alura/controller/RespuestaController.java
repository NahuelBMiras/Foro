...
@SecurityRequirement(name = "bearer-key")  // Seguridad con token bearer
public class RespuestaController {
    ...
    // Crear una nueva respuesta para un tópico
    @PostMapping
    public ResponseEntity<DatosConfirmacionRespuesta> nuevaRespuesta(@RequestBody @Valid DatosNuevaRespuesta datos,
        ...
        URI url = uriComponentsBuilder.path("/topicos/{topicoId}/{respuestaId}")
                                     .buildAndExpand(respuesta.getTopico().getId(), respuesta.getId())
                                     .toUri();
    ...
    public ResponseEntity<DatosLimitadosRespuesta> actualizarSolucionRespuesta(@RequestBody DatosSolucionRespuesta datos){
        ...
        if (topico.getStatus().equals("Cerrado")){
